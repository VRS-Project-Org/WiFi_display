...
void wifiConfig(){
  ...
  wifiMulti.addAP("wifi_hostname1", "wifi_password1"); 
  wifiMulti.addAP("wifi_hostname2", "wifi_password2");  
...
void displayNextPixel(){   // to lcd
  ...
  if(index_ < imgBuffer.length()){
    uint8_t count = 0;
    String dataTx="";
    // one pixel has format R;G;B;      255;21;108;
...
void requestNextPart(){
  ...
  http.begin("http://"+nodeJs_IP+":3000/sendNextPartOfImg");  //Specify request destination
